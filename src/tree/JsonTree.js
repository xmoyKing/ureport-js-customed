...
import JsonDatasetDialog from '../dialog/JsonDatasetDialog.js';
import BaseTree from './BaseTree.js';

export default class DatabaseTree extends BaseTree{
    constructor(container,datasources,ds,jsonDialog,context){
        super();
        this.type='json';
    ...
    _buildDatasource(){
        ...
        const _this=this;
        const jsonDatasetDialog=new JsonDatasetDialog(this.datasources);
        $.contextMenu({
            selector:'#'+this.id,
            callback:function(key,options){
                if(key==='add'){
                    const span=$(options.selector); // 此行？
                    jsonDatasetDialog.show(function(dataset){
                        dataset.fields = [];
                        _this.datasets.push(dataset);
                        _this.addDataset(dataset);
                    });
                }else if(key==='delete'){
                    ...
                    _this.jsonDialog.show(function(name){
                        _this.name=name;
    ...
    addDataset(dataset){
        ...
        li.append(fieldsUL);
        const _this=this;
        const jsonDatasetDialog=new JsonDatasetDialog(this.datasources,this.beanId);
        ...
        $.contextMenu({
            ...
            callback:function(key,options){
                if(key==='add'){
                    ...
                    jsonDatasetDialog.show(function(newDataset){
