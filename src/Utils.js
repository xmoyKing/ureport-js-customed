...
export function tableToXml(context) {
    ...
    console.log('datasources, context', datasources, context);
        ...
        if (type === 'jdbc') {
            ...
            for (let dataset of datasource.datasets) {
                ...
                if (oy.headers) {
                    headerStr = `
                    <headers>
                        ${oy.headers.headers.map(i => `<header name="${i.name}" value="${i.value}"/>`).join('')}
                    </headers>
                    `
                }
                ...
                ds +=
                    `
                <dataset name="${oy.name}" type="json" url="${oy.url}" parameter="${oy.parameter}" method="${oy.method}">
                    ${headerStr}
                    ${jfStr}
                    ${dataset.fields.map(i => `<field name="${i.name}"/>`).join('')}
                </dataset>
                `
